...
for font_dir in font_dirs:
    output_dir = os.path.abspath(f"./output/output_{font_dir}")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    # 이미지 생성 명령 실행
    command = f"python tools/hangul-image-generator.py --label-file labels/256-common-hangul.txt --font-dir ./fonts/{font_dir} --output-dir {output_dir}"
    subprocess.run(command, shell=True, stderr=subprocess.DEVNULL)  # 에러 메시지 무시

    # labels-map.csv 파일 읽기
    labels_map_file = os.path.join(output_dir, "labels-map.csv")
...
with open("./output/labels-map.csv", 'w', encoding='utf-8') as merged_file:
